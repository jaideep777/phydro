...
int main(){
	...
	ifstream fin("tests/test_data/ppfd.tsv");
	double nerr = 0; int count = 0;

	for (auto ppfd : seq(200, 1500, 30)){

		auto res = phydro::phydro_analytical(tc, tc, ppfd, vpd, co2, elv, fapar, kphio, psi_soil, rdark, 3.0, par_plant, par_cost);
		
		cout << setw(10) <<  ppfd      << "\t"; cout.flush();
