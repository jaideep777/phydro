...
rphydro_analytical_r = function(tc, tgrowth, ppfd, vpd,
                                co2, elv, fapar,
  ...
  rphydro::rphydro_analytical(tc, tgrowth, ppfd, vpd,
                              co2, elv, fapar,
                              kphio, psi_soil, rdark,
                              vwind,
                              par_plant_crown,
...
par_plant_leaf = list(conductivity=0.3e-16, psi50=-1.2, b=1, quantile(phydro_input_subdaily$tc, c(.75)));
...
inst.subdaily = test.6day %>%
  ...
  purrr::pmap_dfr(.f = rphydro_instantaneous_analytical_r,
                   ...
                   par_plant = par_plant_leaf,
                   par_cost = par_cost,
                   options = options %>% list_modify(et_method = "ET_DIFFUSION")) %>%
...
inst.subdaily %>% 
  ...
  # filter(a>0) %>%
  ggplot() +
  geom_line(aes(x=time, y=GPP_DT_VUT_REF), col="yellow3") +
  # geom_line(aes(x=as.POSIXct(time), y=GPP_NT_VUT_REF), col="black") +
  geom_line(aes(x=time, y=a), col="red", alpha=0.7) +
  theme_classic()
...
inst.subdaily %>% 
  ...
  # filter(a>0) %>%
  ggplot() +
  geom_line(aes(x=time, y=GPP_NT_VUT_REF), col="yellow3") +
  # geom_line(aes(x=as.POSIXct(time), y=GPP_NT_VUT_REF), col="black") +
  geom_line(aes(x=time, y=a), col="red", alpha=0.7) +
  theme_classic()
```
