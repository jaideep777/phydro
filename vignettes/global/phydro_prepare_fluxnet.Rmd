...
```{r plot_comparison}
df_lsm %>% filter(date >= as.Date("2020-06-01") &
                  date <= as.Date("2020-06-03") ) %>% 
  with(plot(GPP~as.POSIXct(time), type="l"))

df_flux %>% filter(date >= as.Date("2020-06-01") &
                   date <= as.Date("2020-06-03") ) %>% 
...
df_lsm_daily %>% filter(date > as.Date("2014-08-01") & date < as.Date("2015-02-01")) %>% 
  with(plot(LAI~time, type="l", col="cyan4", ylim=c(0,5))) 
df_flux_24hr_mean %>% filter(date > as.Date("2014-08-01") & date < as.Date("2015-02-01")) %>% 
  with(points(LAI~time, type="l", col="blue", ylim=c(0,5))) 
df_flux %>% filter(date > as.Date("2014-08-01") & date < as.Date("2015-02-01")) %>% 
  with(points(LAI~time, type="l", col="pink", ylim=c(0,5))) 
...
test.3day = df_flux %>% filter(date >= as.Date("2020-06-01") &
                               date <= as.Date("2020-06-03") ) 

test.1day = df_flux %>% filter(date == as.Date("2020-06-01"))

test.3day.daily.custom = test.3day %>% group_by(date) %>% do(aggregate_daily_fluxnet(.))

test.3day %>% select(time, SW_IN_F_MDS, TA_F_MDS, VPD_F_MDS, GPP_NT_VUT_REF, GPP_DT_VUT_REF, LE_F_MDS, LAI, FPAR) %>% 
  melt("time") %>% 
  mutate(type="hourly") %>% 
...
df_daytime %>% filter(date > as.Date("2011-01-01") & 
                      date < as.Date("2021-01-01") ) %>%
  select(time, SW_IN_F_MDS, TA_F_MDS, VPD_F_MDS, GPP_NT_VUT_REF, GPP_DT_VUT_REF, LE_F_MDS, LAI, FPAR) %>% 
  melt("time") %>% 
  ggplot(aes(y=value, x=as.POSIXct(time))) + 
...
if (recompute_daily_rollmean_ts){
  df_daily_rollmean = df_daytime %>% select(SW_IN_F_MDS, TA_F_MDS, VPD_F_MDS, CO2_F_MDS, WS_F, FPAR, LAI)
...
df_daily_rollmean %>% 
  ...
  mutate(type="rollavg") %>%
  rbind(df_daytime %>% 
          select(time, SW_IN_F_MDS, TA_F_MDS, VPD_F_MDS, CO2_F_MDS, FPAR, LAI) %>% 
