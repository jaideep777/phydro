...
save(df_flux, file = paste0("~/Downloads/flux_data_kit_beta/",df_meta[[1]]$sitename,"_flux_hh.Rdata"))
```


```{r}
plot(df_flux_24hr_mean$NETRAD~df_flux_24hr_mean$date, type="l")
plot(df_flux_24hr_mean$LAI~df_flux_24hr_mean$date, type="l")

df_flux_24hr_mean %>% select(SW_IN_F_MDS, SW_OUT, LW_IN_F_MDS, LAI, FPAR, time) %>% 
...
df_flux_24hr_mean %>% select(SW_IN_F_MDS, SW_OUT, LW_IN_F_MDS, LAI, P_F, time) %>% 
  mutate(albedo = SW_OUT/SW_IN_F_MDS) %>% 
...
df_flux_24hr_mean %>% select(SW_IN_F_MDS, SW_OUT, LW_IN_F_MDS, LAI, time) %>% 
  mutate(albedo = SW_OUT/SW_IN_F_MDS) %>% 
  filter(albedo > 0 & albedo < 1) %>% 
...
df_flux_24hr_mean %>% select(LW_IN_F_MDS, TA_F_MDS, time) %>% 
...
```{r plot_comparison}
date_start_3day = as.Date("2015-08-01")
date_end_3day = as.Date("2015-08-03")

df_lsm %>% filter(date >= date_start_3day &
                  date <= date_end_3day ) %>% 
  with(plot(GPP~as.POSIXct(time), type="l"))

df_flux %>% filter(date >= date_start_3day &
                   date <= date_end_3day ) %>% 
...
datemid = df_lsm_daily$date[which(dLAI > 1 | df_lsm_daily$LAI > 10)[length(which(dLAI > 1))/2]]
date_start_lai = datemid - 60
date_end_lai = datemid + 60

if (length(datemid) > 0){
df_lsm_daily %>% filter(date > date_start_lai & date < date_end_lai) %>% 
  with(plot(LAI~time, type="l", col="cyan4", ylim=c(0,11))) 
df_flux_24hr_mean %>% filter(date > date_start_lai & date < date_end_lai) %>% 
  with(points(LAI~time, type="o", col="blue", ylim=c(0,11))) 
df_flux %>% filter(date > date_start_lai & date < date_end_lai) %>% 
...
test.3day = df_flux %>% filter(date >= date_start_3day &
                               date <= date_end_3day ) 

test.1day = df_flux %>% filter(date == date_start_3day)
...
```{r}
dates_longterm = df_flux_24hr_mean$date %>% as.numeric() %>% quantile(probs = c(0.25, 0.75)) %>% as.Date()
...
df_daytime %>% filter(date > dates_longterm[1] & 
                      date < dates_longterm[2] ) %>%
  select(time, SW_IN_F_MDS, NETRAD, TA_F_MDS, VPD_F_MDS, GPP_NT_VUT_REF, GPP_DT_VUT_REF, LE_F_MDS, LAI, FPAR) %>% 
  melt("time") %>% 
  ggplot(aes(y=value, x=as.POSIXct(time))) + 
  geom_line(col="aquamarine4") + 
...
if (recompute_daily_rollmean_ts){
  df_daily_rollmean = df_daytime %>% select(SW_IN_F_MDS, NETRAD, TA_F_MDS, VPD_F_MDS, CO2_F_MDS, WS_F, FPAR, LAI) %>% mutate(NETRAD = ifelse(is.na(NETRAD), 0, NETRAD))
...
df_daily_rollmean %>% 
  ...
  mutate(type="rollavg") %>%
  rbind(df_daytime %>% 
          select(time, date, SW_IN_F_MDS, NETRAD, TA_F_MDS, VPD_F_MDS, CO2_F_MDS, FPAR, LAI) %>% 
          filter(date > dates_longterm[1] & 
                 date < dates_longterm[2] ) %>%
          select(-date) %>% 
          melt("time") %>% 
...
plot_inputs(start_date = date_start_3day, end_date = date_start_3day+180)

plot_inputs(start_date = date_start_3day, end_date = date_start_3day+12)
