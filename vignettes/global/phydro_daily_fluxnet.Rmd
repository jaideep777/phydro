...
aggregate_daily = function(df){
  ...
  max_t = as.POSIXct(maxppfd$time)
  df_aroundmax = df %>% filter(as.POSIXct(time) < (max_t + 3*3600) & 
                               as.POSIXct(time) > (max_t - 3*3600) )
...
par_plant_leaf = list(conductivity=0.16e-16, psi50=-1.2, b=1);
