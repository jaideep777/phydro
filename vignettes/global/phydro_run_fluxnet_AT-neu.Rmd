...
site_meta = read.csv(file = paste0("~/Downloads/flux_data_kit_beta/",siteID,"_meta.csv"))

load(paste0("~/Downloads/flux_data_kit_beta/",siteID,"_flux_daytime_mean.Rdata"))

load(paste0("~/Downloads/flux_data_kit_beta/",siteID,"_phydro_input_acclim.Rdata"))

load(paste0("~/Downloads/flux_data_kit_beta/",siteID,"_phydro_input_daily_24hr_mean.Rdata"))

load(paste0("~/Downloads/flux_data_kit_beta/",siteID,"_phydro_input_subdaily.Rdata"))

load(paste0("~/Downloads/flux_data_kit_beta/",siteID,"_flux_daytime_rollmean_7.Rdata"))

load(paste0("~/Downloads/flux_data_kit_beta/",siteID,"_flux_24hr_mean.Rdata"))
...
t_extent = df_daytime %>% ungroup() %>% select(date) %>% as_tibble() %>% slice(1, nrow(.)) %>% pull(date)
  
plot_inputs(start_date = as.Date(t_extent[1]), end_date = as.Date(t_extent[2]))
...
par_cost = list(alpha=0.06, gamma=1);
...
# test.6day %>%
#   filter(date >= "2005-08-21" & date <= "2005-08-27") %>% 
#   select(-date) %>% 
...
# 
# test.6day %>%
#   filter(date >= "2013-04-21" & date <= "2013-04-27") %>% 
#   select(-date) %>% 
#   left_join(df_flux %>% select(time, GPP_DT_VUT_REF, GPP_NT_VUT_REF)) %>% 
#   melt("time") %>%
#   ggplot(aes(y=value, x=as.POSIXct(time))) +
...
# test.6day %>%
#   filter(date >= "2018-10-21" & date <= "2018-10-27") %>% 
#   select(-date) %>% 
#   left_join(df_flux %>% select(time, GPP_DT_VUT_REF, GPP_NT_VUT_REF)) %>% 
#   melt("time") %>%
#   ggplot(aes(y=value, x=as.POSIXct(time))) +
...
dat %>% 
  filter(date >= "2005-04-21" & date <= "2005-04-27") %>% 
  with(plot(SW_IN_F_MDS~time, type="l", col="orange", ylim=c(0,1000), ylab="SW_IN"))
dat %>% 
  filter(date >= "2005-04-21" & date <= "2005-04-27") %>% 
  with(matplot(y=cbind(a, GPP_DT_VUT_REF), x=time, type="l", lty=1, lwd=c(1,3), ylim=c(0,25), ylab="GPP", col=c("black",scales::alpha("green3", 0.5))))
...
dat %>% 
  filter(date >= "2005-10-21" & date <= "2005-10-27") %>% 
  with(plot(SW_IN_F_MDS~time, type="l", col="orange", ylim=c(0,1000), ylab="SW_IN"))
dat %>% 
  filter(date >= "2005-10-21" & date <= "2005-10-27") %>% 
  with(matplot(y=cbind(a, GPP_DT_VUT_REF), x=time, type="l", lty=1, lwd=c(1,3), ylim=c(0,25), ylab="GPP", col=c("black",scales::alpha("green3", 0.5))))
...
d = inst.subdaily %>% 
  ...
  summarize(gpp = mean(GPP_DT_VUT_REF, na.rm=T),
            ...
            le = mean(le+0.55*le_s_wet, na.rm=T)) %>% 
  drop_na 
