...
dat_pml = tibble(tc = -5:55) %>% mutate(dat = purrr::map(.x=tc, .f = ~rphydro_analytical(.x, ppfd, vpd, co2, elv, fapar, kphio, psi_soil, rdark, par_plant, par_cost, list_modify(options, ftemp_vj_method = "FV_kattge07") ))) %>% unnest_wider(dat)
```
