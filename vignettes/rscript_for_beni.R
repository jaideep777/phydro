...
acc1 = rphydro_analytical(tc, tc, ppfd, netrad, vpd_acc, co2, elv, fapar, kphio, 0, rdark, vwind, par_plant_acclim, par_cost, options)

dat1 = list(vpd = exp(seq(log(100),log(5000),length.out=50)),
           psi_soil = c(0, -2.5)) %>%
  cross_df() %>% 
  mutate(dat = purrr::map2(.x=psi_soil, .y=vpd, .f = ~rphydro_instantaneous_analytical(acc1$vcmax25, acc1$jmax25, tc, tc, ppfd, netrad, .y, co2, elv, fapar, kphio, .x, rdark, vwind, par_plant_acclim, par_cost, options))) %>% 
  unnest_wider(dat)
...
acc2 = rphydro_analytical(tc, tc, ppfd, netrad, vpd_acc, co2, elv, fapar, kphio, 0, rdark, vwind, par_plant_no_acclim, par_cost, options)

dat2 = list(vpd = exp(seq(log(100),log(5000),length.out=50)),
           psi_soil = c(0, -2.5)) %>%
  cross_df() %>% 
  mutate(dat = purrr::map2(.x=psi_soil, .y=vpd, .f = ~rphydro_instantaneous_analytical(acc2$vcmax25, acc2$jmax25, tc, tc, ppfd, netrad, .y, co2, elv, fapar, kphio, .x, rdark, vwind, par_plant_no_acclim, par_cost, options))) %>% 
  unnest_wider(dat)
