...
list(ga = exp(seq(log(0.01), log(12), length.out=20)), gs=exp(seq(log(0.01), log(0.4), length.out=20))) %>% cross_df() %>% 
  mutate(dat1 = purrr::map2_dbl(.x=ga, .y=gs, .f = ~calc_transpiration_pml(ppfd/2, .y, .x, 25, 1.01e5, vpd))) %>%
...
```{r}
list(ga = exp(seq(log(0.01), log(12), length.out=20)), gs=exp(seq(log(0.01), log(0.4), length.out=20))) %>% cross_df() %>% 
  mutate(dat1 = purrr::map2_dbl(.x=ga, .y=gs, .f = ~calc_transpiration_pml(ppfd/2, .y, .x, 25, 1.01e5, 0))) %>%
  ggplot(aes(x=gs, y=dat1, group=ga, col=ga)) + 
    geom_line() + 
...
```{r}
list(ga = exp(seq(log(0.01), log(12), length.out=20)), gs=exp(seq(log(0.01), log(0.4), length.out=20))) %>% cross_df() %>% 
  mutate(dat1 = purrr::map2_dbl(.x=ga, .y=gs, .f = ~calc_transpiration_pml(0, .y, .x, 25, 1.01e5, vpd))) %>%
  ggplot(aes(x=gs, y=dat1, group=ga, col=ga)) + 
    geom_line() + 
    geom_line(aes(y=1.6*gs*vpd/1.01e5), col="red") + 
    scale_colour_gradient(trans = "log") + 
...
```{r}
list(ga = exp(seq(log(0.01), log(12), length.out=20)), gs=exp(seq(log(0.01), log(0.4), length.out=20))) %>% cross_df() %>% 
  mutate(trans = purrr::map2_dbl(.x=ga, .y=gs, .f = ~calc_transpiration_pml(ppfd/2, .y, .x, 25, 1.01e5, vpd))) %>%
  mutate(gs1 = purrr::map2_dbl(.x=ga, .y=trans, .f = ~calc_gs_pml(ppfd/2, .y, .x, 25, 1.01e5, vpd))) %>%
...
```{r}
list(ga = exp(seq(log(0.01), log(12), length.out=20)), gs=exp(seq(log(0.01), log(0.4), length.out=20))) %>% cross_df() %>% 
  mutate(dedgs = purrr::map2_dbl(.x=ga, .y=gs, .f = ~calc_dE_dgs_pml(ppfd/2, .y, .x, 25, 1.01e5, vpd))) %>%
  mutate(dedgs_num = purrr::map2_dbl(.x=ga, .y=gs, .f = ~calc_dE_dgs_pml_num(ppfd/2, .y, .x, 25, 1.01e5, vpd))) %>%
