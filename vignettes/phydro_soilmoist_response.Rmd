...
```{r}
dat = tibble(psi_soil = seq(-6,0,length.out=50)) %>% mutate(dat = purrr::map(.x=psi_soil, .f = ~rphydro_analytical(tc, ppfd, vpd, co2, elv, fapar, kphio, .x, rdark, par_plant, par_cost, options))) %>% unnest_wider(dat)
```
...
dat_inst = tibble(psi_soil = seq(-6,0,length.out=50)) %>% mutate(dat = purrr::map(.x=psi_soil, .f = ~rphydro_instantaneous_analytical(acc$vcmax25, acc$jmax25, tc, ppfd, vpd, co2, elv, fapar, kphio, .x, rdark, par_plant, par_cost, options))) %>% unnest_wider(dat)
...
```{r}
dat_apx = tibble(psi_soil = seq(-6,0,length.out=50)) %>% mutate(dat = purrr::map(.x=psi_soil, .f = ~rphydro_analytical(tc, ppfd, vpd, co2, elv, fapar, kphio, .x, rdark, par_plant, par_cost, list_modify(options, gs_method = "GS_APX2")))) %>% unnest_wider(dat)
```
...
dat_pml = tibble(psi_soil = seq(-6,0,length.out=50)) %>% mutate(dat = purrr::map(.x=psi_soil, .f = ~rphydro_analytical(tc, ppfd, vpd, co2, elv, fapar, kphio, .x, rdark, par_plant, par_cost, options))) %>% unnest_wider(dat)
```
