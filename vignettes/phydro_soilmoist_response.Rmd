...
```{r}
dat = tibble(psi_soil = seq(-6,0,length.out=50)) %>% mutate(dat = purrr::map(.x=psi_soil, .f = ~rphydro_analytical(tc, ppfd, vpd, co2, elv, fapar, kphio, .x, rdark, vwind, par_plant, par_cost, options))) %>% unnest_wider(dat)
```
...
dat_inst = tibble(psi_soil = seq(-6,0,length.out=50)) %>% mutate(dat = purrr::map(.x=psi_soil, .f = ~rphydro_instantaneous_analytical(acc$vcmax25, acc$jmax25, tc, ppfd, vpd, co2, elv, fapar, kphio, .x, rdark, vwind, par_plant, par_cost, options))) %>% unnest_wider(dat)
...
```{r}
dat_apx = tibble(psi_soil = seq(-6,0,length.out=50)) %>% mutate(dat = purrr::map(.x=psi_soil, .f = ~rphydro_analytical(tc, ppfd, vpd, co2, elv, fapar, kphio, .x, rdark, vwind, par_plant, par_cost, list_modify(options, gs_method = "GS_APX2")))) %>% unnest_wider(dat)
```

```{r}
df.m2 <- dat_apx %>% select(-profit) %>% melt("psi_soil")
...
dat_pml = tibble(psi_soil = seq(-6,0,length.out=50)) %>% mutate(dat = purrr::map(.x=psi_soil, .f = ~rphydro_analytical(tc, ppfd, vpd, co2, elv, fapar, kphio, .x, rdark, vwind, par_plant, par_cost, options))) %>% unnest_wider(dat)
```

```{r}
df.pml <- dat_pml %>% select(-profit) %>% melt("psi_soil")
...
dat_pml2 = list(psi_soil = seq(-6,0,length.out=50), vwind=exp(seq(log(0.01),log(10),length.out=10))) %>% cross_df() %>% mutate(dat = purrr::map2(.x=psi_soil, .y=vwind, .f = ~rphydro_analytical(tc, ppfd, vpd, co2, elv, fapar, kphio, .x, rdark, .y, par_plant, par_cost, options))) %>% unnest_wider(dat)
```

```{r fig.width=9}
dat_pml2 %>% select(-profit) %>% melt(c("psi_soil", "vwind")) %>% 
